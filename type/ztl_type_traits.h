...
namespace ztl
{
	...
	public:
		typedef ValueType value_type;
		typedef value_wrapper<value_type, Val> self_type;
	public:
		static const value_type value = Val;
		operator value_type()
		{
...
#undef IS_INTEGRAL
	...
	struct replace_type
	{
		typedef typename replace_type_impl<Tc, Ts, Tr, ztl::is_same<Tc, Ts>::value>::type type;
	};
	/*右值转发*/
		...
		move(Type&& target)
	{
			...
			//return ((typename remove_reference<Type>::type&&)target);
	};
	...
	{
		ztl::remove_reference<typename ztl::remove_cv<Type>::type>::type temp = ztl::move(left);
		left = ztl::move(right);
		right = ztl::move(temp);
	}
	/*template<class Type> inline
	...
	struct is_iterator
		: public integral_constant<bool, !ztl::is_integral<_Iter>::value>
